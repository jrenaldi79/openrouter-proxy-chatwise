...
describe('Real API Integration Tests', () => {
  ...
  describe('Real OpenRouter API Tests', () => {
    ...
    testIf(hasRealApiKey)(
      'should get real credits from OpenRouter API',
      async () => {
      ...
      10000
    ); // Longer timeout for real API

    testIf(hasRealApiKey)(
      'should get real auth key info from OpenRouter API',
      async () => {
        const response = await request(app)
          .get('/v1/auth/key')
          .set('Authorization', validApiKey);
    ...
    testIf(hasRealApiKey)(
      'should get real models from OpenRouter API',
      async () => {
        const response = await request(app)
          .get('/v1/models')
          .set('Authorization', validApiKey);
    ...
    testIf(hasRealApiKey)(
      'should handle real API errors properly',
      async () => {
    ...
    testIf(hasRealApiKey)(
      'should handle chat completions with real API',
      async () => {
        const response = await request(app)
    ...
    test('should skip real API tests when no API key is available', () => {
      if (!hasRealApiKey) {
        console.log(
          'Skipping real API tests - no OPENROUTER_TEST_API_KEY found'
        );
  ...
  describe('Performance with Real API', () => {
    ...
    testIf(hasRealApiKey)(
      'should maintain reasonable response times with real API',
      async () => {
